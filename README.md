...
- **Web e-reader (site-wide catalog)**: Optional **`public/reader/library-curated.json`** (portable library format) is merged into each visitor’s IndexedDB once per **`BUNDLED_READER_CATALOG_VERSION`** ([`bundled-library-seed.ts`](src/lib/reader/bundled-library-seed.ts)); studio shows **Load site catalog** and explains that the default shelf is per-browser unless you deploy that file (only redistribute works you have rights to host). See [`public/reader/README.txt`](public/reader/README.txt).
- **Web e-reader (chapter structure hints)**: After **Detect chapters**, the import preview runs **local heuristics** ([`chapter-import-analyze.ts`](src/lib/reader/chapter-import-analyze.ts)) — tiny-chapter runs, duplicate titles, long chapters with embedded heading-like lines — with one-click **Merge** using [`mergeChapterIndexRange`](src/lib/reader/text-chapters.ts). Optional **`POST /api/reader/suggest-chapter-structure`** sends **short excerpts only** to OpenRouter (set **`READER_CHAPTER_LLM_KEY`** or **`OPENROUTER_API_KEY`**, optional **`READER_CHAPTER_LLM_MODEL`**) for conservative AI merge suggestions; nothing auto-applies without the user clicking Merge (avoids silently wrecking real chapters).
- **Web e-reader (import fidelity)**: [`paragraph-format.ts`](src/lib/reader/paragraph-format.ts) **`normalizeLinesKeepVerticalStructure`** + EPUB [`normalizeBlockText`](src/lib/reader/epub-extract-server.ts) preserve **blank lines** (verse / `<br>` chains); import only collapses horizontal spaces/tabs and may split **very long** lines at sentence/word boundaries — **no** wording shortening, abbreviation expansion, or paraphrase. PDF reflow still drops isolated **page-number** lines as layout noise; **`B ook` → `Book`** remains a pdf.js run-repair only.
...
- **Web e-reader (EPUB)**: `POST /api/reader/extract-epub` + `createEpubImportDraft` — unzip OPF spine, XHTML → plain paragraphs (`fflate`, `fast-xml-parser`, `node-html-parser`); `npm run verify:epub-fixture` regenerates `e2e/fixtures/minimal-reader-test.epub` and smoke-tests import; E2E `e2e/e-reader-epub.spec.ts`.
