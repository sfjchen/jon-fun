...
- **Web e-reader AI chapter hints** (`POST /api/reader/suggest-chapter-structure`): **`GEMINI_API_KEY`** or **`GOOGLE_GENERATIVE_AI_API_KEY`** (Google Gemini API, default model **`gemini-3.1-flash-lite-preview`**; override with **`READER_CHAPTER_GEMINI_MODEL`**). If no Gemini key, use **`READER_CHAPTER_LLM_KEY`** or **`OPENROUTER_API_KEY`** (OpenRouter; **`READER_CHAPTER_LLM_MODEL`** defaults to `openai/gpt-4o-mini`). **`READER_CHAPTER_LLM_PROVIDER`**: `google` \| `openrouter` to force one backend when both keys exist.
