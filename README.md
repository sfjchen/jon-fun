...
- **UVIMCO Notes** (`/games/uvimco-notes`): Meeting shorthand workspace — left **Meetings** sidebar (one note page per meeting), `?term` / line-`?` AI lookups, CodeMirror editor, Notion-like light UI (Lato), Windows shortcuts (Ctrl+…), multi-session localStorage + Supabase sync, admin at `/admin/uvimco-notes`
...
- **UVIMCO Notes**: Notion-like light theme (high-contrast `#37352f` on white), **Meetings** left sidebar (one page per meeting), CodeMirror editor, streaming AI panel, Ctrl shortcuts, localStorage + Supabase sync; **always run `npm run test:e2e:uvimco-visual` after UI changes** (deployed sfjc.dev, editor height + screenshots — local mocked tests miss layout bugs).
