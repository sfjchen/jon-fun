...
- **Web e-reader**: *The Last Wish* (`Witcher 1…Sapkowski…epub` under `e2e/fixtures/`) added to **`seed:reader-portable`**, **`sync:reader-communal-fixtures`**, and **[`reader-fixture-profiles.ts`](scripts/reader-fixture-profiles.ts)**; **`build:reader-curated`** appends it to **`library-curated.json`** when the file exists; **`BUNDLED_READER_CATALOG_VERSION`** **3**.
