...
- **Notes AI** (`POST /api/uvimco-notes/lookup`): **`OPENROUTER_API_KEY`** or **`GEMINI_API_KEY`** / **`GOOGLE_GENERATIVE_AI_API_KEY`**. Defaults: **`UVIMCO_NOTES_LOOKUP_MODEL`** = `google/gemini-2.5-flash-lite`; **`UVIMCO_NOTES_DECODE_MODEL`** = `google/gemini-2.5-flash`. Server **auto-fallback** on 503/overload (flash-lite → flash → alternate provider). **`UVIMCO_NOTES_LLM_PROVIDER`**: `google` | `openrouter`. Trigger: **`?term` + space or Enter** (not just space). Smoke: **`npm run smoke:uvimco-notes-llm`**; deploy lookup E2E: **`npm run test:e2e:uvimco-lookup`**. Session sync → **`uvimco_note_sessions`**.
...
- **Notes**: Notion-like light UI; full-width editor; collapsible **Notes + AI** side panel; UI prefs in localStorage; `/games/notes` alias; Supabase table **`uvimco_note_sessions`** live; AI lookup **`?term` + space/Enter** with model fallback on 503; **fix:** cloud sync no longer clears in-flight AI lookup; status bar shows **? terms** (shorthand flags); large screenshots stripped on cloud sync; **`npm run verify:uvimco-notes-triggers`** + **`test:e2e:uvimco-lookup`** + sync/visual deploy E2E.
