...
- **Jeopardy (saved library)**: Passcode-gated `GET /api/jeopardy/library` lists `*.jeopardy.json` files in [`data/jeopardy/`](data/jeopardy); `POST /api/jeopardy/library/[filename]/import` clones one into a new Supabase collab board. Passcode defaults to **`1234`**, overridable via **`JEOPARDY_LIBRARY_PASSCODE`**. `next.config.mjs` `outputFileTracingIncludes` ships `data/jeopardy/**` with the serverless bundle on Vercel. Landing page exposes a "📚 Saved library" section: enter passcode → click any saved board → fresh collab board opens at `/games/jeopardy/edit/[slug]`. Unlocked passcode cached in localStorage; **Lock** button clears it.
