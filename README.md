...
- **Notes** (`/games/notes` → `/games/uvimco-notes`): Shorthand note-taking with **`line?` / `line??` AI triggers**; global search (Ctrl+Shift+F); tags + meeting datetime; sync key + restore; autoglossary + todo rollup; resizable panel; **Summarize** (Ctrl+K); **CodeMirror editor by default** (Tiptap optional via `NEXT_PUBLIC_NOTES_WYSIWYG=1`); design doc **`docs/NOTES-DESIGN.md`**; admin at `/admin/uvimco-notes`
...
- **Notes AI** (`POST /api/uvimco-notes/lookup`): **`OPENROUTER_API_KEY`** or **`GEMINI_API_KEY`** / **`GOOGLE_GENERATIVE_AI_API_KEY`**. Triggers: **`line?`** (instant, debounced) and **`line??`** (section). Context assembly injects glossary + sources + related notes. Defaults: **`UVIMCO_NOTES_LOOKUP_MODEL`** = `google/gemini-2.5-flash-lite`; **`UVIMCO_NOTES_DECODE_MODEL`** = `google/gemini-2.5-flash`. Smoke: **`npm run smoke:uvimco-notes-llm`**; E2E: lookup/sync/search/restore deploy specs.
...
- **Notes platform evolution (Phase 1–3)**: New trigger grammar (`?` / `??`, no `?term`); **GlobalSearch**; **SyncPanel** + `restoreFromServer`; tags/metadata; **react-markdown** AI panel; autoglossary + todo rollup; resizable panel; **Sources** memory bank (local + Supabase); context assembler; **Summarize** rename; **CodeMirror default** (Tiptap opt-in); migration **`notes_memory`** applied via Supabase MCP; **`memorySync.ts`** for glossary/sources cloud sync; agent rule **`.cursor/rules/supabase-migrations.mdc`**.
