...
- **Notes** (`/games/notes` → `/games/uvimco-notes`): Shorthand note-taking with `?term` AI lookups; full-width editor; **Notes + AI** in collapsible right panel (panel hidden by default); localStorage + Supabase sync; admin at `/admin/uvimco-notes`
...
- **Exceptions** (different shell / full-bleed): **Pear Navigator** (own layout), **Chwazi mobile** (non-notebook chrome for touch UI), **Poker lobby & table** (full-bleed main—masthead still **sfjc.dev** → home), **Notes** (standalone Lato workspace — minimal back link only). Inner game pages share the same **line-paper + tall** header as `/` (see `PageShell.tsx`).
...
- **Notes**: Notion-like light UI; full-width editor; collapsible **Notes + AI** side panel; UI prefs in localStorage; `/games/notes` alias; Supabase table **`uvimco_note_sessions`** live on sfjchen's Project; **`npm run test:e2e:uvimco-sync`** + **`npm run test:e2e:uvimco-visual`** after UI/sync changes (deployed sfjc.dev).
